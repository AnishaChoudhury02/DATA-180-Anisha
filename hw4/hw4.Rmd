...
output: 
  html_document
---
17/20. Interpret your graphs. You are not answering the questions asked.
...
```{r}
statisticSummary <-records %>% summary()
statisticSummary
```
# Question 2
...
```{r}
recordBroken <- records %>% filter(record_duration>365) 
count(recordBroken)
```
# Question 3
...
```{r}
longestTime <- records %>% filter(record_duration == max(record_duration)) %>% select(player, track, date, record_duration)
longestTime
```
...
head(recordTime)
fastestTrack <- recordTime %>% filter(meanTime == min(meanTime)) 
fastestTrack <- fastestTrack$track
fastestTrack
#Mario Raceway
```
# Question 5
...
```{r}
year <- substr(records$date, 1, 4)
newRecords <- mutate(records, Year=year)
head(newRecords)
```
...
```{r, fig.width=10}
library(ggplot2)
newTime %>% ggplot(aes(Year,avgTime)) + geom_bar(stat='identity')
...
```{r}
records %>% select(type,time)  %>% ggplot(., aes(x= time, fill=type,alpha=.5)) + geom_histogram() 
...
```{r}
time_per <- records %>% filter(shortcut=="No") %>% mutate(time_per=ifelse(type=="Three Lap",time/3,time)) %>% ggplot(., aes(time_per, fill=type)) + geom_histogram(position="identity",alpha=.5) 
time_per
```
