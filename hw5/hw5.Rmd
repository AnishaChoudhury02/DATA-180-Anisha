...
Vans_Euclidean
```
The vans with the shortest distance between them are the Aerostar and the Caravan (3.44). The vans with the largest distance between them are the Lumina APV and the Eurovan (61.80).


c) Repeat the steps in part (b) but use the Manhattan metric and call the object `Vans_Manhattan`.
```{r}
Vans_Manhattan <- Vans93[,-1]
Vans_Manhattan <- dist(Vans_Manhattan, method = "manhattan")
...
a) Nearest-neighbor
d(A,B) = min{d12,d13,d15,d42,d43,d45}
       = min{1,9,1.6,6,5,2} = 1 

b) Farthest-neighbor
d(A,B) = max{d12,d13,d15,d42,d43,d45}
       = max{1,9,1.6,,6,5,2} = 9

c) Average-neighbor
d(A,B) = 1/|A||B| Σd(a,b) = 1/2·3 [1+9+1.6+6+5+2] = 24.6/6 = 4.1
...
```{r}
vertebrate <- read.csv("/Users/anishachoudhury/Desktop/DATA 180 Anisha/DATA-180-Anisha/hw5/vertebrate.csv")
vertebrate
```
...
plot(Ver_Euclidean,main="Clustered vertebrates, Euclidean Metric,Single Linkage",cex=.75,xlab="Vertebrates",lwd=3,cex.lab=1.25,hang=-1,sub="")

```

(ii)using the Manhattan metric with single linkage and
```{r}
Ver_Manhattan<-as.matrix(dist(vertebrate[,2:7], method = "manhattan")) #skip column 1 and 8
...
Ver_Manhattan
plot(Ver_Manhattan,main="Clustered vertebrates, ManhattanMetric, Single Linkage",cex=.75,xlab="Vertebrates",lwd=3,cex.lab=1.25,hang=-1,sub="")
```

Which method do you prefer?
Manhattan as the numbers are rounded so it is easier to interpret.
