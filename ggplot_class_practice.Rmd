...
ggplot(diamonds, aes(x=carat,y=price)) + 
                          geom_point() + 
                          labs(y = 'price $', x = 'carat value')  #first comp is x and y, 2nd comp how u want to plot i.e box plot or histogram, etc, 3rd comp is the labels
```
...
```{r,}
ggplot(diamonds, aes(x=cut,y=price)) + geom_boxplot() #aes are the aesthetics which are given parameters
```
...
```{r}
ggplot(diamonds, aes(x=carat,y=price)) + geom_point(alpha = 0.2) + (geom_point(col = "orange")) #alpha control the intensity of the colors
```
